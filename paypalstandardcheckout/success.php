...
file_put_contents('success_dump_Post.txt', print_r($_POST, true));
file_put_contents('success_dump_Get.txt', print_r($_GET, true));
file_put_contents('success_dump_Request.txt', print_r($_REQUEST, true));
...
// If transaction data (website own payment parameters & paypal payment parameters) is available in the URL 
if(!empty($_GET['cartids']) && !empty($_GET['userid']) && !empty($_GET['tx']) && !empty($_GET['amt']) && !empty($_GET['cc']) && !empty($_GET['st']) && !isset($_SESSION["SavedTransactionRecord"]) && !empty($_GET['transactionDate'])){ 
    // Get transaction information from URL 
    $item_number = $_GET['cartids'];  
    $userid = $_GET['userid']; 
    $txn_id = $_GET['tx']; 
    ...
    $prevPaymentResult = $conn->query("SELECT * FROM paypaltransaction WHERE Paypal_Token = '".$txn_id."' and User_ID='".$userid."'"); 
    ...
    if($prevPaymentResult->num_rows > 0){ 
		...
		$query = "SELECT *,(select Service_Name from services where Service_ID=cart.Service_ID) as 'Service_Name', (select Service_Price from services where Service_ID=cart.Service_ID) as 'Service_Price', (Select Category_Name from category where Category_ID = (SELECT Category_ID FROM `services` WHERE Service_ID=cart.Service_ID)) as 'Category_Name' FROM `cart` where cart_id in (".$item_number.") and User_ID='".$userid."'";
		$productResult = $conn->query($query); //"SELECT * FROM cart WHERE cart_id in (".$item_number.")"
		//$productRow = $productResult->fetch_assoc(); 

        if ($productResult->num_rows > 0) {
            while ($row = $productResult->fetch_assoc()) {
                $sql4 = "INSERT INTO `orders`(`Service_ID`, `User_ID`, `Order_Price`, `Payment_ID`, `Payment_Method`, `Order_Date`, `filePath`, `Order_Status`) VALUES ('".$row["Service_ID"]."','".$row["User_ID"]."','".$row["Service_Price"]."','".$txn_id."','PayPal',NOW(),'".$row["filePath"]."','Pending')";
                $result4 = $conn->query($sql4);

                $sql5 = "Delete From cart where Cart_ID in (".$item_number.")";
	...
	$_SESSION["SavedTransactionRecord"] = true;
...
} // If transaction data (only website own payment parameters & no paypal payment parameters) is available in the URL 
elseif(!empty($_GET['cartids']) && !empty($_GET['userid']) && !empty($_GET['carttotal']) && !isset($_SESSION["SavedTransactionRecord"])   && !empty($_GET['transactionDate'])){ 
    // Get transaction information from URL 
    $item_number = $_GET['cartids'];  
	$userid = $_GET['userid']; 
	$txn_id = $_GET['cartids'].'|'.$_GET['userid'].'|'.$_GET['transactionDate'];
    //$txn_id = $_GET['tx']; 
    //$payment_gross = $_GET['amt']; 
    //$currency_code = $_GET['cc']; 
    //$payment_status = $_GET['st']; 
    ...
    // Check if transaction data exists with the same TXN ID. 
    //$prevPaymentResult = $conn->query("SELECT * FROM paypaltransaction WHERE Paypal_Token = '".$txn_id."'"); 
	...
	$insert = $conn->query("INSERT INTO `paypaltransaction`(`Paypal_Token`, `Cart_IDs`, `User_ID`, `Transaction_Status`, `Date`) VALUES ('".$txn_id."','".$item_number."','".$userid."','',NOW())");
	$payment_id = $conn->insert_id;
...
/*else{
    ...
    $query = "SELECT *,(select Service_Name from services where Service_ID=cart.Service_ID) as 'Service_Name', (select Service_Price from services where Service_ID=cart.Service_ID) as 'Service_Price', (Select Category_Name from category where Category_ID = (SELECT Category_ID FROM `services` WHERE Service_ID=cart.Service_ID)) as 'Category_Name' FROM `cart` where cart_id in (3,4) and User_ID='2'";
    $productResult = $conn->query($query); //"SELECT * FROM cart WHERE cart_id in (".$item_number.")"
    $productRow = $productResult->fetch_assoc();
    $payment_id = 1;

    $message = '<div class="vc_column-inner"><div class="wpb_wrapper"><div class="rb_info_box success"><div class="icon_wrapper"><i></i></div><div class="content_wrapper"><p class="h5 info_box_title">Thank you for your payment</p></div></div></div></div>.';
    //header( "refresh:3;url=../order-history.php" );
}*/  
?>
...
<div class="container">
    <div class="status">
        <?php if(!empty($payment_id)){ ?>
            ...
            <p><b>Name:</b> <?php echo $row['Service_Name']; ?></p>
            <p><b>Price:</b> <?php echo $row['Service_Price']; ?></p>
            <?php } ?>
        <?php }else{ ?>
...
========================================================*/
		...
		.vc_custom_1572870659576 {
			...
			border-left-color: #ffaf00 !important;
...
<body class="page-template-default page page-id-910 theme-seoes woocommerce-no-js wpb-js-composer js-comp-ver-6.0.5 vc_responsive" data-boxed="false" data-default="false" itemscope="itemscope" itemtype="http://schema.org/WebPage">
					...
					<div data-vc-full-width="true" data-vc-full-width-init="false" class="vc_row wpb_row vc_row-fluid vc_custom_1571319827010 vc_row-has-fill vc_row-o-content-middle vc_row-flex">
						...
						<div id='rb_column_5de228592ceac' class='rb_column_wrapper vc_col-sm-6 '>
							<div class="wpb_column vc_column_container vc_col-sm-6">
								<div class="vc_column-inner">
									<div class="wpb_wrapper">
										<div id='rb_icon_list_5de228592d04e' class='rb_icon_list_module header_icons direction_line'><a href='tel:+305111222333%20' class='custom_url'><i class='flaticon-telephone-auricular-with-cable'></i><span class='title'>+(305) 111-222-333 </span></a><a href='mailto:info@seoservices.com' class='custom_url'><i class='flaticon-email'></i><span class='title'>info@seoservices.com</span></a></div>
									</div>
								</div>
							</div>
						</div>
										...
										<div id='rb_title_area_5de228593362b' class='custom page_title_container  mouse_anim scroll_anim shared_bg' style=""><img data-depth='0.80' src='../wp-content/uploads/2013/06/title_hexagons.png' class='page_title_dynamic_image' alt='Contacts' />
											<div class='page_title_wrapper'>
												<div class="page_title_customizer_size">
													<h1 class='page_title'>Thanks</h1>
												</div><span class='title_divider'></span>
												<div class="breadcrumbs">
													<div class="container">
														<nav class="bread-crumbs"><a href="index.php" property="v:title">Home</a><span class='delimiter'></span><span class="current">Thanks</span></nav><!-- .breadcrumbs -->
													</div>
												</div>
											</div>
										</div>
		...
		<div id="site-header-mobile" class="site-header-mobile">
			<div class="header-content">
				...
				<div class="page_title_container masked" style="">
					...
					<img data-depth="0.80" src="../wp-content/uploads/2013/06/title_hexagons.png" class="page_title_dynamic_image" alt="Thanks" />
											...
											<div class="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1570620243054">
												<div id='rb_column_5de228593fa6a' class='rb_column_wrapper vc_col-sm-4 '>
													<div class="wpb_column vc_column_container vc_col-sm-4">
														<div class="vc_column-inner">
															<div class="wpb_wrapper">
																<div id='rb_icon_list_5de228593fb99' class='rb_icon_list_module header_icons direction_column mobile_align_center'><a href='tel:3053335522' class='custom_url'><i class='flaticon-telephone-auricular-with-cable'></i><span class='title'>(305) 333-5522</span></a><a href='mailto:info@seoservices.com' class='custom_url'><i class='flaticon-email'></i><span class='title'>info@seoservices.com</span></a></div>
															</div>
														</div>
													</div>
												</div>
									...
									<div class="wpb_wrapper">
										<div class='rb_textmodule_5de2285940917 rb_textmodule mobile_align_center'>
											<div class='rb_textmodule_content_wrapper'>
												<center>
													<p>Copyright © SeoServices 2019 — All rights reserved.</p>
												</center>
											</div>
										</div>
									</div>
	...
	<style id='rb-footer-inline-css' type='text/css'>
		...
		#rb_testimonials_5de228593bb49.style_with_bg .testimonial {
			background-image: linear-gradient(0deg, #FF6849, #FFAF00, #fff, #fff);
		}
