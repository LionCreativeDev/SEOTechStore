...
if ( $PaymentOption == "PayPal" && isset($_POST["btnReview"]) && $_POST["btnReview"] === "Review" && isset($_POST["token"]) )
{
	/*
	'------------------------------------
	...
	*/
	...
	if( $ack == "SUCCESS" || $ack == "SUCCESSWITHWARNING" )
	{
		...
		*/
		...
		$sql1 = "Select * from paypaltransaction where Paypal_Token = '".$_POST["token"]."'";
		$result1 = $conn->query($sql1);
		
		if ($result1->num_rows > 0) {
			while ($row1 = $result1->fetch_assoc()) {
				//Update Trascation Status in PaypalTransaction Table
				$sql2 = "Update paypaltransaction set Transaction_Status = '".$ack."' where Paypal_Token = '".$_POST["token"]."'";
				$result2 = $conn->query($sql2);
				//Update Trascation Status in PaypalTransaction Table

				//Insert Data from Cart To Order Table
				$sql3 = "SELECT * , (select Service_ID from services where Service_ID=Cart.Service_ID) as 'Service_ID', (select Service_Price from services where Service_ID=Cart.Service_ID) as 'Service_Price' FROM `cart` where User_ID='" . $row1["User_ID"] . "' and Cart_ID in (".$row1["Cart_IDs"].")";
				$result3 = $conn->query($sql3);

				if ($result3->num_rows > 0) {
					while ($row3 = $result3->fetch_assoc()) {
						$sql4 = "INSERT INTO `orders`(`Service_ID`, `User_ID`, `Order_Price`, `Payment_ID`, `Payment_Method`, `Order_Date`, `filePath`, `Order_Status`) VALUES ('".$row3["Service_ID"]."','".$row3["User_ID"]."','".$row3["Service_Price"]."','".$row1["PaypalTransaction_ID"]."','PayPal',NOW(),'".$row3["filePath"]."','Pending')";
						$result4 = $conn->query($sql4);

						$sql5 = "Delete From Cart where Cart_ID in (".$row1["Cart_IDs"].")";
